...
void loop() {
  ...
  if (rcvCnt > 0) {
    int index = split(rcvBuff, ',', pieces);
    if(index == 3) {
      int areaPosi = pieces[0].toInt();
      if ( (0 < areaPosi) && (areaPosi <= NUM_DISP_VALUES) ) {
        pieces[1].trim(); //remove top & bottom space of label
        labels[areaPosi-1] = pieces[1];
        if (pieces[2].length() > MAX_VALUE_STRLEN) {
          values[areaPosi-1] = pieces[2].substring(0, MAX_VALUE_STRLEN) + "...";
        }
        else {
          values[areaPosi-1] = pieces[2];
        }
      }
    }
